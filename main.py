...
def img_object_detection_to_json(file: str):
    """
    Обнаружение объектов на изображении.

    Аргументы:
        file (str): Файл изображения в формате base64.
    Возвращает:
        dict: Результат в формате JSON, содержащий обнаруженные уведомления.
    """
    # Шаг 1: Инициализация словаря результата с пустыми значениями
    result = {'notification_windows': []}

    # Шаг 2: Преобразование строки base64 в байты
    binary_image = base64.b64decode(file)

    # Шаг 3: Преобразование файла изображения в объект изображения
    input_image = get_image_from_bytes(binary_image)

    # Шаг 4: Предсказание моделью
    predict = detect_sample_model(input_image)

    # Шаг 5: Фильтрация результатов детекции на основе известных названий объектов
    filtered_results = predict[predict['name'].isin(['block', 'append', 'close'])]

    # Шаг 6: Подготовка JSON-ответа
    for index, row in filtered_results.iterrows():
        notification = {
            'order': index + 1,
            'type': 'unknown',
            'coordinate': {
                'top_left': {'x': round(row['xmin'], 3), 'y': round(row['ymin'], 3)},
                'bottom_right': {'x': round(row['xmax'], 3), 'y': round(row['ymax'], 3)}
            },
            'buttons': [
                {
                    'name': row['name'],
        ...
        result['notification_windows'].append(notification)

    # Шаг 7: Журналирование и возврат результата
    logger.info("Результат: {}", result)
    return result


@app.post("/img_object_detection_to_img")
def img_object_detection_to_img(file: str):
    """
    Object Detection from an image plot bbox on image

    Args:
        file (str): The image file in base64 format.
    Returns:
    ...
    # Step 2: Convert the bytes to an image object
    input_image = get_image_from_bytes(binary_image)

    # Step 3: Predict from model
    predict = detect_sample_model(input_image)

    # Step 4: Add bbox on image
    final_image = add_bboxs_on_img(image=input_image, predict=predict)

    # Step 5: Return image in base64 format
    img_bytes = get_bytes_from_image(final_image)
